...
const getPrimaryImage = (
  ...
  const gallery = Array.isArray(product.mediaGallery) ? product.mediaGallery : [];
  if (gallery.length === 0) return null;

  const first = gallery[0];
  if (
    first &&
    typeof first.asset === "object" &&
    first.asset !== null &&
    "url" in first.asset &&
    typeof first.asset.url === "string"
  ) {
    return {
      url: first.asset.url,
      alt:
        (typeof first.asset.alt === "string" && first.asset.alt.length > 0
          ? first.asset.alt
