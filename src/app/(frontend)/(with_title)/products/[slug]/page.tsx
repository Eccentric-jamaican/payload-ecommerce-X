...
import { serializeLexical } from "@/app/(frontend)/(with_title)/blog/[slug]/lexical-serializer";
...
export async function generateMetadata({
  ...
  const heroMedia =
    Array.isArray(product.mediaGallery) && product.mediaGallery[0]
      ? product.mediaGallery[0]
      : null;

  const heroAsset =
    heroMedia &&
    typeof heroMedia.asset === "object" &&
    heroMedia.asset !== null
      ? (heroMedia.asset as Media)
      : null;

  return {
    title: `${product.name} | Alphamed Global`,
    description: product.shortDescription ?? undefined,
    openGraph: heroAsset?.url
      ? {
          title: product.name,
          description: product.shortDescription ?? undefined,
...
const resolveMedia = (
  entry: Product["mediaGallery"][number] | undefined,
): MediaWithUrl | null => {
  ...
  if (
    ...
    entry.asset.url.length > 0
  ) {
    return entry.asset as MediaWithUrl;
  }
  return null;
};

const ProductsSlugPage = async ({ params }: ProductParams) => {
  const { slug } = await params;
  if (!slug) notFound();
  ...
  const additionalImages =
    Array.isArray(product.mediaGallery) && product.mediaGallery.length > 1
      ? product.mediaGallery
          ...
          .filter((asset): asset is MediaWithUrl => Boolean(asset))
      : [];
        ...
        <header className="grid gap-10 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] md:items-start">
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-[#4F5563]">
              {product.productCode ? <span>Product code: {product.productCode}</span> : null}
